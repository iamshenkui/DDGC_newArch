...
test.describe("browser smoke: fidelity gates", () => {
  test("replay boot → full meta-loop → fidelity → viewport", async ({ page }) => {
    ...
    // Town → Expedition Planning → Provisioning → Dungeon Hint → Expedition → Dungeon Assist → Dungeon Map → Combat → Result → Return → Town
    ...
    await expect(
      ...
      "Expedition planning screen must be visible"
    ).toBeVisible();
    await expect(
      page.getByText("位面探索"),
    ...
    await expect(
      page.locator(".plane-selection-strip"),
  ...
  test("live boot exercises live bridge path", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="expedition-planning-screen"]'),
      "Live expedition planning screen must be visible"
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "位面探索" }),
