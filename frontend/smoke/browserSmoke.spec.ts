...
test.describe("browser smoke: fidelity gates", () => {
  test("replay boot → full meta-loop → fidelity → viewport", async ({ page }) => {
    ...
    await expect(
      page.getByText("可选位面"),
      "Plane selection strip title must be visible"
    ).toBeVisible();
    await expect(
    ...
    // Select a different plane to exercise plane selection
    await page.locator('[data-testid="plane-card-baihu"]').click();
    await settle(page);

    // Remove a hero and re-add to exercise assignment toggle
    await page.locator('[data-testid="party-slot-hero-hero-hunter-01"]').click();
    await settle(page);
    await page.locator('[data-testid="roster-hero-hero-hunter-01"]').click();
    await settle(page);
  ...
  test("live boot exercises live bridge path", async ({ page }) => {
    ...
    // Full live flow: expedition-planning → provisioning → dungeon-hint → expedition → dungeon-assist → dungeon-map → combat → result → return
    await page.getByRole("button", { name: "Return to Town" }).click();
    await page.waitForSelector(".town-viewport", { timeout: 5_000 });
    await settle(page);

    // Launch expedition planning from live
    await page.locator(".estate-embark-button").click();
    await page.waitForSelector('[data-testid="expedition-planning-screen"]', { timeout: 5_000 });
    await settle(page);
    ...
    // Exercise plane selection and hero assignment, then proceed
    await page.locator('[data-testid="plane-card-baihu"]').click();
    await settle(page);
    await page.locator('[data-testid="party-slot-hero-hero-hunter-live-01"]').click();
    await settle(page);
    await page.locator('[data-testid="roster-hero-hero-hunter-live-01"]').click();
    await settle(page);
    await page.getByRole("button", { name: "前往战前补给" }).click();
    await settle(page);
    ...
    await expect(
      page.getByRole("heading", { name: "战前补给" }),
      "Live provisioning title (战前补给) must be visible"
    ).toBeVisible();
  ...
  test("combat attack flow reaches success result", async ({ page }) => {
    ...
    // Boot replay and go through expedition-planning -> provisioning -> expedition -> combat
    await page.getByRole("button", { name: "Boot Replay" }).click();
    await page.waitForSelector(".town-viewport", { timeout: 8_000 });
    await settle(page);
    ...
    // Proceed to provisioning from expedition planning
    await page.getByRole("button", { name: "前往战前补给" }).click();
    await settle(page);
