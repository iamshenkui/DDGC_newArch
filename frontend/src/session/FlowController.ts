...
export type ScreenKey = "startup" | "loading" | "town" | "hero-detail" | "building-detail" | "inventory" | "provisioning" | "expedition" | "result" | "return" | "unsupported" | "fatal";
...
export function resolveScreen(snapshot: DdgcFrontendSnapshot): ScreenKey {
  ...
  if (snapshot.viewModel.kind === "inventory") {
    return "inventory";
  }
...
export function canTransition(
  ...
  switch (intent.type) {
    ...
    case "open-inventory":
      if (screen !== "town") {
        return { allowed: false, reason: "open-inventory is only valid in town" };
      }
      return { allowed: true };
