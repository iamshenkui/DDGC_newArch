...
export function canTransition(
  ...
  switch (intent.type) {
    ...
    case "open-dungeon-items":
      if (screen !== "dungeon-interaction" && screen !== "dungeon-map" && screen !== "combat") {
        return { allowed: false, reason: "open-dungeon-items is only valid in dungeon-interaction, dungeon-map, or combat" };
      }
      return { allowed: true };
    ...
    case "select-dungeon-item":
      ...
      }
      {
        ...
        if (!item) {
          return { allowed: false, reason: `dungeon item ${intent.itemId} does not exist` };
        }
      }
      return { allowed: true };
    ...
    case "use-dungeon-item":
      ...
      }
      {
        ...
        if (item.qty <= 0) {
          return { allowed: false, reason: `dungeon item ${intent.itemId} is out of stock` };
        }
        if (!item.isUsable) {
          return { allowed: false, reason: `dungeon item ${intent.itemId} is not usable` };
        }
        if (intent.heroId && !snapshot.viewModel.party.some((hero) => hero.id === intent.heroId)) {
          return { allowed: false, reason: `target hero ${intent.heroId} does not exist` };
        }
      }
      return { allowed: true };
