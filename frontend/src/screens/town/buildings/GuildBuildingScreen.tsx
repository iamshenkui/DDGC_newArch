...
import { GuildTrialSelectionPanel } from "./GuildTrialSelectionPanel";
...
export const GuildBuildingScreen: Component<GuildBuildingScreenProps> = (props) => {
  ...
  const [activeTab, setActiveTab] = createSignal<"upgrade" | "use">("upgrade");
  ...
  const hasRoster = () => (vm().roster?.length ?? 0) > 0;
  ...
  return (
    <div class="app-frame" data-source-scene="Assets/Scenes/EstateManagement.unity">
      ...
      {/* ── Tab Bar — mirrors GuildWindow/TabBar (升级设施 / 使用设施) ── */}
      {hasRoster() && (
      ...
      {activeTab() === "upgrade" ? (
        ...
        /* ── Use Facility Tab — Trial Selection ── */
        <GuildTrialSelectionPanel
          roster={vm().roster ?? []}
          onSelectHero={(heroId) => {
            props.onAction(`guild-trial-select:${heroId}`);
          }}
          onTalk={() => {
            props.onAction("guild-trial-talk");
          }}
          onLeave={() => {
