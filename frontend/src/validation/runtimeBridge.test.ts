...
import type { BuildingDetailViewModel, HeroDetailViewModel, ProvisioningViewModel, ExpeditionSetupViewModel, DungeonContentViewModel, ExpeditionResultViewModel, ReturnViewModel } from "../bridge/contractTypes";
...
describe("provisioning and expedition launch flow", () => {
  ...
  it("replay launch-expedition transitions to dungeon-content state", async () => {
    const bridge = new ReplayRuntimeBridge();
    await bridge.boot();
    await bridge.dispatchIntent({ type: "start-provisioning" });
    ...
    expect(snapshot.flowState).toBe("dungeon-content");
    expect(snapshot.viewModel.kind).toBe("dungeon-content");
    const dcVm = snapshot.viewModel as DungeonContentViewModel;
    expect(dcVm.kind).toBe("dungeon-content");
    expect(dcVm.isEnterable).toBe(true);
  });

  it("replay enter-dungeon-map transitions to result state", async () => {
    const bridge = new ReplayRuntimeBridge();
    await bridge.boot();
    await bridge.dispatchIntent({ type: "start-provisioning" });
    await bridge.dispatchIntent({ type: "confirm-provisioning" });
    ...
    const snapshot = await bridge.dispatchIntent({ type: "enter-dungeon-map" });
  ...
  it("live launch-expedition transitions to dungeon-content state", async () => {
    const bridge = new LiveRuntimeBridge();
    await bridge.boot();
    await bridge.dispatchIntent({ type: "start-provisioning" });
    await bridge.dispatchIntent({ type: "confirm-provisioning" });
  ...
  it("live enter-dungeon-map transitions to result state", async () => {
    const bridge = new LiveRuntimeBridge();
    await bridge.boot();
    await bridge.dispatchIntent({ type: "start-provisioning" });
    await bridge.dispatchIntent({ type: "confirm-provisioning" });
  ...
  it("town -> provision -> launch path is reproducible in replay", async () => {
    ...
    expect(launchSnapshot.flowState).toBe("dungeon-content");
    expect(launchSnapshot.viewModel.kind).toBe("dungeon-content");

    const resultSnapshot = await bridge.dispatchIntent({ type: "enter-dungeon-map" });
