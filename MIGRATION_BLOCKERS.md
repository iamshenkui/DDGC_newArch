...
- **Status:** Resolved — Phase 2 implemented `ConditionAdapter` in `src/run/conditions.rs` with `FirstRound`, `StressAbove`, `StressBelow`, `DeathsDoor`, `TargetHasStatus`, and `ActorHasStatus` conditions. Phase 4a added `HpAbove`, `TargetHpAbove`, and `TargetHpBelow` HP-threshold conditions (US-802-a arbiter, US-802-c docs, US-802-d tests), `InMode` dungeon-mode condition (US-803-a contracts, US-803-b state-layer mode field, US-803-c arbiter, US-803-d adapter, US-803-e docs, US-803-f tests), and `OnKill` kill-trigger condition (US-804-b arbiter, US-804-d tests). `InMode` resolves from `ConditionContext::dungeon` via `crate::contracts::dungeon_mode_name()` and also checks an optional `mode: Option<String>` field on `ConditionContext` for game-layer mode overrides (e.g. boss room battle modifiers). It is used by the Hunter's `xuanwu_strike` skill. `OnKill` checks kill records via `ConditionContext::actor_killed_enemy()` and is used by the Hunter's `executioner_strike` skill. Framework-native conditions (`Probability`, `IfTargetHealthBelow`, `IfActorHasStatus`) are bridged through the adapter with identical behavior. `IfTargetPosition` returns `Unknown` (formation context unavailable). All condition families with migrated content are implemented; remaining DDGC variants are low-impact deferred with no migrated content dependencies.
