...
- **Status:** Resolved — Phase 2 implemented `ConditionAdapter` in `src/run/conditions.rs` with `FirstRound`, `StressAbove`, `StressBelow`, `DeathsDoor`, `TargetHasStatus`, and `ActorHasStatus` conditions. Phase 4a added `HpAbove`, `TargetHpAbove`, and `TargetHpBelow` HP-threshold conditions (US-802), and `InMode` dungeon-mode condition (US-803). `InMode` resolves the current dungeon from `ConditionContext::dungeon` via `dungeon_mode_name()` and is used by the Hunter's `xuanwu_strike` skill. Framework-native conditions (`Probability`, `IfTargetHealthBelow`, `IfActorHasStatus`) are bridged through the adapter with identical behavior. `IfTargetPosition` returns `Unknown` (formation context unavailable).
